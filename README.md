...
UniLP Lens mendukung **22 jaringan blockchain EVM** secara langsung dengan konfigurasi RPC publik terverifikasi, Chain ID resmi, dan smart contract PositionManager untuk Uniswap V3 maupun V4:

| Network | Chain ID | Uniswap Ver. | Official PositionManager Address | Explorer |
| :--- | :---: | :---: | :--- | :--- |
| **Monad Mainnet** | `143` | V3 / V4 | `0x3Bb14E3D0Cd50aBe3EdACa06d06c29C78676C31A` (V4)<br>`0xC36442b4a4522E871399CD717aBDD847Ab11FE88` (V3) | [Monadscan](https://monadscan.com) |
| **MegaETH** | `4326` | V3 | `0xC36442b4a4522E871399CD717aBDD847Ab11FE88` | [MegaScan](https://mega.etherscan.io) |
| **Arc Mainnet** | `5042` | V3 | `0x6049c9a0e26405C0985f9E3685C87d0aE917f82B` | [Explorer Arc](https://explorer.arc.io) |
...
2. **Dukungan Multichain 22+ Jaringan:**
   - Pilihan chain preset instan termasuk Monad Mainnet (`143`), MegaETH (`4326`), Arc Mainnet (`5042`), Tempo (`4217`), Robinhood Chain (`4663`), Ethereum, Base, Arbitrum, dan lainnya.
...
4. **Deteksi Status Posisi & Rentang Tick Visual:**
   - Menampilkan badge status berwarna dinamis:
     - 🟢 **IN RANGE**: Posisi aktif dengan likuiditas yang menghasilkan biaya swap.
     - 🟡 **OUT OF RANGE**: Posisi aktif namun harga pool berada di luar rentang tick.
     - ⚪ **CLOSED**: Posisi telah ditarik / diburn.
...
Styled with a **single-viewport desktop layout (zero window scrolling)** and an **Obsidian Black & Electric Neon Lime (#bcfb00)** color scheme, it delivers ultra-fast position inspection with direct on-chain contract queries.

---

### ✨ Key Features

- **Universal Input & Instant Uniswap URL Parser:** Paste any Uniswap URL (`https://app.uniswap.org/positions/v3/base/12345`) or enter raw token IDs. The parser automatically detects the chain, protocol version, and token ID.
- **22+ EVM Networks Supported:** Pre-configured presets for Monad Mainnet (`143`), MegaETH (`4326`), Arc Mainnet (`5042`), Tempo (`4217`), Robinhood (`4663`), Ethereum, Base, Arbitrum, Optimism, Polygon, BNB, Avalanche, Celo, Blast, zkSync, Linea, World Chain, Zora, Soneium, Unichain, Ink, and X Layer.
- **Single-Viewport Layout:** No whole-page scrolling. Left sidebar hosts all inputs and configs; right panel delivers fluid internal scrolling.
- **Rich Status Badges & Tick Ranges:** Instant visual indicators for `IN RANGE`, `OUT OF RANGE`, and `CLOSED`.
...
* **EVM (Ethereum, Base, Arbitrum, Monad, etc.):**  
  `0xbfcba220bc538af067fd74e74ae59a9eb1b94bf6`
* **Solana:**  
  `Ae2BrzfsSsRekrX17kAMd2NM69jupVPeheRETsmXPDFh`
* **Sui:**  
  `0xab8e9a1a3e191e2db9d967004e993b0d1aafeb1dcc23615843713a7c7b56b0d1`
* **Bitcoin:**  
  `bc1q390kc993fw8jx5l876vth7jcw4j99e30kppq7j`
